...
#
# PLATINUM STANDARD - Reusable UI Components
# This module provides standardized, high-level functions for rendering
# consistent UI elements, such as headers and KPI cards, across the application.
...
def render_metric_card(
    ...
    `analytics.aggregation.calculate_kpi_statistics`.

    Args:
        title: The title for the metric card.
        stats: A dictionary containing statistics (mean, ci, delta, p-value, etc.).
        kpi_format: The format string for the main value.
        unit_prefix: A prefix for the value (e.g., '$').
        unit_suffix: A suffix for the value (e.g., '%').
    """
    # --- Main Value Formatting ---
    main_value = stats.get('current_mean')
    if main_value is not None:
        value_str = f"{unit_prefix}{kpi_format.format(main_value)}{unit_suffix}"
    else:
        value_str = "N/A"

    # --- Delta Formatting & Coloring ---
    delta_str = "No comparison period"
    delta_val = stats.get('delta_pct')
    delta_color = "off"
    if delta_val is not None:
        delta_str = f"{delta_val:+.1%}"
        # Set color only if change is statistically significant
        if stats.get('is_significant'):
            is_positive_change = stats.get('is_positive_change')
            if is_positive_change is True:
                delta_color = "normal"  # Green for positive change
            elif is_positive_change is False:
                delta_color = "inverse" # Red for negative change
    ...
    # --- Render the native Streamlit Metric ---
    st.metric(
        label=title,
        value=value_str,
        delta=delta_str,
...
def render_dashboard_filter_form(
    ...
    (Placeholder for a more complex component if needed).

    Args:
        df: The DataFrame to derive filter options from.
        date_col: The name of the date column for the date range filter.
        default_days: The default number of days for the date range.
    ...
    date_range = st.sidebar.date_input(
        ...
        min_value=min_date,
        max_value=max_date,
        key=f"filter_date_range_{st.session_state.get('current_page', '')}"
    )
    ...
    if len(date_range) == 2:
        ...
        # Fallback if the user deselects a date
        filters['start_date'] = default_start
        filters['end_date'] = max_date
        
    # Example for adding another filter (e.g., Zone)
    # if 'zone_name' in df.columns:
    #     zone_options = ["All"] + sorted(df['zone_name'].unique())
    #     filters['zone'] = st.sidebar.selectbox("Zone:", zone_options)
